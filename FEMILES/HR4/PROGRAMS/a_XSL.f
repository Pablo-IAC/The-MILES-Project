...
c XSL
      PARAMETER (npxi=106301) !XSL #pixels in the spectra
...
ccccccccccccccccccccc
	open(99,file='../D/INPUT/GALAXY_CABECERAS_XSL',status='old') !XSL
	do k=1,61
...
c     ==================== END IGIMF OPTION ===========================
	   DO iao=1,ntold
	    ageo=ttold(iao)
          call STU_XSL(izold(izo),ageo,BETAo,realeo,ZMISOo,wwffio,
...
cc     & fxUmsy,fxUisy,fxUsy,fxUis004y)
	     call STU_XSL(izyng(izy),agey,BETAy,realey,ZMISOy,wwffiy,
...
c		 write(77,*)wfis(lup,1),wfis(lup,2)/wfi(lup,2)
		 write(77,*)wfis(lup,1),wfis(lup,2)
	      ...
	      call e_XSL(alpha,izold(izo),realeo,izyng(izy),realey) !XSL
2325	      continue
