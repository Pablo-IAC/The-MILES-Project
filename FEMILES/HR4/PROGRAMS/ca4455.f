c*******SUBRUTINA DE Ca4455************************************************
c        SUBROUTINE ca4455(nra,i,W,stars)
        SUBROUTINE ca4455(i,W,stars)
...
c        feh(i)=fez(i)+dlog10(ra(nra,3)/ra(1,3))
	spend=(4455.-4400.)/(5500.-4400.)
	W=0.0
	stars=0.0
	if(jotai(i).eq.0)goto 50004
	DO j=1,jotai(i)
	...
	ZMAG=-3.3037+5.4191e-3*at-9.8489e-7*at**2
			elseif((lc.ge.4).and.(R(i,j,1).le.7.0))then
	ZMAG=3.4906-0.1027*R(i,j,1)-0.03137*R(i,j,1)**2
			else
				goto 19004
			endif
		elseif((R(i,j,3).ge.3.5527).and.(R(i,j,3).le.3.7076))then
	ZMAG=1.5326+11.8034*t+0.7298*feh(i)+0.0026*feh(i)**3
		elseif((R(i,j,3).gt.3.7076).and.(R(i,j,3).le.4.1225))then
	t=10**t
	ZMAG=-0.7362+2.3650*t*t+0.1721*feh(i)**2+0.8484*t*feh(i)
		else
			goto 19004
		endif
        if(j.eq.1)then
