...
c	DOUBLE PRECISION a(4,4),bb(4),sson(4)
	...
	DIMENSION pizold(15),pizyng(15),yter(11),ygir(7)
...
c para calcular valores [Fe/H]
	...
	DATA yter/.245d0,.245d0,.246d0,.246d0,.248d0,.251d0,.256d0,
     & .259d0,.2734d0,.288d0,.303d0/
      CALL FNORM
        write(*,*)'WRITE THE CHANGE OF TEMPERATURE OF THE GIANTS'
	read(*,*)giadif
...
c
	...
	close(99)
cccc
	open(99,file='./INPUT/GALAXY_CABECERAS_BLUE',status='old')
	do k=1,61
		read(99,'(A80)')cab(k,1)
	enddo
	close(99)
	open(99,file='./INPUT/GALAXY_CABECERAS_RED',status='old')
	do k=1,61
		read(99,'(A80)')cab(k,2)
	enddo
	close(99)
	open(99,file='./INPUT/GALAXY_CABECERAS_CAT',status='old')
	do k=1,61
		read(99,'(A80)')cab(k,3)
	enddo
	close(99)
