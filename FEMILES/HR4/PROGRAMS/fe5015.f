c*******SUBRUTINA DE Fe5015************************************************
c        SUBROUTINE fe5015(nra,i,W,stars)
        SUBROUTINE fe5015(i,W,stars)
...
c        feh(i)=fez(i)+dlog10(ra(nra,3)/ra(1,3))
	spend=(5015.-4400.)/(5500.-4400.)
	W=0.0
	stars=0.0
	if(jotai(i).eq.0)goto 40010
	DO j=1,jotai(i)
	...
	ZMAG=-136.58+1.2586e-1*at-2.3065e-5*at**2
			elseif((lc.ge.4).and.(R(i,j,1).le.7.0))then
	ZMAG=15.270-5.0518*R(i,j,1)+0.58055*R(i,j,1)**2
			else
				goto 00010
			endif
		elseif((R(i,j,3).ge.3.5527).and.(R(i,j,3).le.3.7076))then
	ZMAG=6.3877+21.1152*t+2.1785*feh(i)-.3128*R(i,j,4)
     &  -43.8834*t*t*R(i,j,4)
		elseif((R(i,j,3).gt.3.7076).and.(R(i,j,3).le.4.1225))then
	t=10**t
	ZMAG=-4.0938+9.1303*t+0.4208*feh(i)**2+2.4945*feh(i)*t
		else
			goto 00010
		endif
        if(j.eq.1)then
