...
cc       
c       if(tiss.lt.10000.0d0)then
c        write(cirtf(1:8),'(A2,A1,F5.0)')'iT','0',tiss
c       else
c        write(cirtf(1:8),'(A2,F6.0)')'iT',tiss
c       endif
c       if(giss.lt.0.0d0)then
c        write(cirtf(9:14),'(A1,A1,F4.2)')'G','m',abs(giss)
c       else
c        write(cirtf(9:14),'(A1,A1,F4.2)')'G','p',abs(giss)
c       endif
c       if(fiss.lt.0.0d0)then
c        write(cirtf(15:20),'(A1,A1,F4.2)')'Z','m',abs(fiss)
c       else
c        write(cirtf(15:20),'(A1,A1,F4.2)')'Z','p',abs(fiss)
c       endif
cc       
c       if(alk(1).lt.10000.0d0)then
c        write(cirtf(21:29),'(A3,A1,F5.0)')'_oT','0',alk(1)
c       else
c        write(cirtf(21:29),'(A3,F6.0)')'_oT',alk(1)
c       endif
c       if(alk(2).lt.0.0d0)then
c        write(cirtf(30:35),'(A1,A1,F4.2)')'G','m',abs(alk(2))
c       else
c        write(cirtf(30:35),'(A1,A1,F4.2)')'G','p',abs(alk(2))
c       endif
c       if(alk(3).lt.0.0d0)then
c        write(cirtf(36:41),'(A1,A1,F4.2)')'Z','m',abs(alk(3))
c       else
c        write(cirtf(36:41),'(A1,A1,F4.2)')'Z','p',abs(alk(3))
c       endif
c	OPEN(43,FILE=cirtf,STATUS='OLD',ERR=6976)
c	CLOSE(43,STATUS='DELETE')
c6976	OPEN(43,IOSTAT=IOS,FILE=cirtf,STATUS='NEW')
cc	do nonoo=1,61
cc	  write(43,'(A80)')cab(nonoo,4)
cc	enddo
c	do lupir=1,npxk
c		write(43,*)(ssppk(lupir,lllll),lllll=1,2)
c	enddo
c	close(43)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc	 
	 fkmil=0.0d0
