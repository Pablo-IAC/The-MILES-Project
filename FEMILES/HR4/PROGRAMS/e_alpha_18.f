...
c*******SUBRUTINA QUE ESCRIBE LOS RESULTADOS******************************
	...
	DIMENSION vsin(50),ZZZ(8),ZISOL(8),fZISOL(8),SBF(50),vSBF(50)
     &  ,DWGI(2,8),fhst(4),fhsbf(4),cfhst(4),cfhsbf(4)
      DIMENSION tilin(9)
...
c
c	write(*,'(A2,1x,6(F7.4,1x))')
c     &  shapa,ZMU-1.d0,zetao,realeo,zetay,reay,alpha
...
c     &  shapa,ZMU-1.d0,zetao,realeo,zetay,reay,alpha,
     ...
     &  shapa,ZMU-1.d0,zetao,realeo,
     &	(vsin(n31),n31=8,nobs)
	endif
c     &	(vsin(n31),n31=1,nobs),(ZISOL(mlum),mlum=1,8)
c	write(*,'(A2,1x,(F5.2,1x),2(F7.4,1x),39(F7.3))')
c     &  shapa,ZMU-1.d0,zetao,realeo,(vsin(n31),n31=1,nobs)
c
c	escribe en out_qua
	write(39,'(A2,1x,(F5.2,1x),2(F7.4,1x),7(F12.8))')shapa,ZMU-1.d0,zetao,realeo
     & ,quaqn/quaq10,quaqn/quaq05,quaqn/quaq15,quaqn,quaq10,quaq05,quaq15
c	write(*,'(A2,1x,(F5.2,1x),2(F7.4,1x),7(F12.8))')shapa,ZMU-1.d0,zetao,realeo
c     & ,quaqn/quaq10,quaqn/quaq05,quaqn/quaq15,quaqn,quaq10,quaq05,quaq15
c
c	escribe en out_ef
      write(31,'(A2,1x,(F5.3,1x),2(F7.4,1x),F7.0,1x,4(F7.3,1x),
     &F7.0,1x,4(F7.3,1x))')
     &shapa,ZMU-1.d0,zetao,realeo,
     &(a90m(nt),nt=1,5),
     &(a90c(nt),nt=1,3)
c   	  write(31,'(A2,1x,(F5.2,1x),2(F7.4,1x),1(F6.0,4(1x,F6.3)),
c   &2(F6.0,2(1x,F6.3)))')
c   &  shapa,ZMU-1.d0,zetao,realeo,zetay,reay,alpha,
c     &	(a90m(nt),nt=1,5),(a90c(nt),nt=1,3)
c     &  ,t222,g222,f222
c
c	escribe en out_dg
...
c	write(*,'(8(1x,F8.3))')(ZISOL(nt),nt=1,8)
c
...
ccc        write(79,'(A2,1x,6(F7.4,1x),(1(f7.3,1x)))')
ccc     &  shapa,ZMU-1.d0,zetao,realeo,zetay,reay,alpha,
ccc     &  CATFIN
c	ENDDO
	return
	end
