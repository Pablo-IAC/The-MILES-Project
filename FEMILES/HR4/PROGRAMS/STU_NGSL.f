      SUBROUTINE STU_NGSL(m,ageSSP,BETA,reale,ZMISO,xm9,xm9s,a9m,f9m,
     &qum,qu10,qu05,qu15)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
c NGSL
      PARAMETER (npxu=10023) !IRTF #pixels in the spectra
      DIMENSION a9m(3),xm9(npxu,2),xm9s(npxu,2)
	...
	DO 1502 kk=1,num(m) !puede subir hasta 150000
      IF(z(m,kk,1).gt.tiso(n)-.0001.and.z(m,kk,1).lt.tiso(n)+.0001)THEN
       if(ncha.le.0)then
        kkeypT=kkeypT+1
	 endif
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
	...
	xmaso=ZMISO
	xmasa=ZMISA
      CALL hrsl_NGSL(m,xm9,xm9s,a9m,f9m,qum,qu10,qu05,qu15)
      RETURN
      END
