      SUBROUTINE sigmam_NGSL(tiss,giss,fiss,ala,aloec)
CC Version 20-November-2022.
C------------------------------------------------------------------------------
...
c range 1160.0-10179.8A, for a given set of atmospheric parameters
C------------------------------------------------------------------------------
C INPUT FILES:
...
C------------------------------------------------------------------------------
      ...
      PARAMETER (npxu=10023) !NGSL
      PARAMETER (nstu=1100)
      DIMENSION sscb(nstu,8),sscpb(nstu,4,8),ssc(nstu,8),sscp(nstu,4,8)
      DIMENSION ala(npxu,2),weigc(nstu,8),cubstc(npxu,8)
c
...
c NGSL+FUV
      CHARACTER*80 staru
      COMMON/aamcou/aau(nstu,4) !NGSL+FUV
      COMMON/hru1/nstaru !NGSL+FUV
      COMMON/hru2/staru(nstu) !NGSL+FUV
...
c TO BE TESTED:
      tz0_f=50000.0d0 !temp mas alta para la que distinguimos [M/H]
c-----------------------------------------------
...
c	    if(fiss.gt.-0.05d0)then
	if(abs(t50/aau(l,1)-tisst).le.dddt.and.
c     &  abs(aau(l,2)-giss).le.dddg.and.aau(1,3).gt.-0.15d0)then
     &  abs(aau(l,2)-giss).le.dddg)then
     	      starsi=starsi+1.0d0
	endif
c	    else
c	if(abs(t50/aau(l,1)-tisst).le.dddt.and.
c     &  abs(aau(l,2)-giss).le.dddg.and.aau(1,3).le.0.0d0)then
c     	      starsi=starsi+1.0d0
c	endif
c	    endif
	   ELSE
	    if(abs(t50/aau(l,1)-tisst).le.dddt
     &       .and.abs(aau(l,2)-giss).le.dddg
     &       .and.abs(aau(l,3)-fiss).le.dddz)then
     	     starsi=starsi+1.0d0
	    endif
	   ENDIF
	  enddo
...
c          write(65,*)dens !util correr con Zsun, 5Gyr para determinar DMA!!!
	    if(dens.gt.DMA)then
       	     sigmat=ddt
             sigmag=ddg
             sigmaz=ddz
...
c BUSQUEDA ESTRELLAS DE ACUERDO A ESTAS SIGMAS PARA 8 CUBOS
	...
	rbandi=0.0d0 !bandera para detener proceso si hay * en 8cubos
	 ...
	 xxs=xs+rntt*fxs !factor que multiplica a sigma
         DO is=1,nstaru !de las estrellas de la libreria
c       __________________________________________________
c	 if (is.ne.nist) then !exclusion estrella a ajustar
c       __________________________________________________
	  IF(nbande(1).eq.0)THEN
          IF(aau(is,1).ge.tiss.and.aau(is,2).ge.giss.and.
     &    aau(is,3).ge.fiss)THEN
          IF(tiss.le.tz0_i.or.tiss.ge.tz0_f)THEN
c	   if(fiss.gt.-0.05d0)then
            if(abs(t50/aau(is,1)-tisst).le.sigmat*xxs.and.
     &      abs(aau(is,2)-giss).le.sigmag*xxs)then
c     &      .and.aau(1,3).gt.-0.15d0)then
              nscb(1)=nscb(1)+1
              sscb(nscb(1),1)=staru(is)
              do kkkkl=1,4
                sscpb(nscb(1),kkkkl,1)=aau(is,kkkkl)
              enddo
...
c	   else
c            if(abs(t50/aau(is,1)-tisst).le.sigmat*xxs.and.
c     &      abs(aau(is,2)-giss).le.sigmag*xxs
c     &      .and.aau(1,3).le.0.0d0)then
c              nscb(1)=nscb(1)+1
c              sscb(nscb(1),1)=staru(is)
c              do kkkkl=1,4
c                sscpb(nscb(1),kkkkl,1)=aau(is,kkkkl)
c              enddo
...
c	   endif
	  ELSE
           if(abs(t50/aau(is,1)-tisst).le.sigmat*xxs.
     &	   and.abs(aau(is,2)-giss).le.sigmag*xxs.and.
     &	   abs(aau(is,3)-fiss).le.sigmaz*xxs)then
              nscb(1)=nscb(1)+1
              sscb(nscb(1),1)=staru(is)
              do kkkkl=1,4
                sscpb(nscb(1),kkkkl,1)=aau(is,kkkkl)
...
c
	  IF(nbande(2).eq.0)THEN
          IF(aau(is,1).lt.tiss.and.aau(is,2).ge.giss.and.
     &      aau(is,3).ge.fiss)THEN
          IF(tiss.le.tz0_i.or.tiss.ge.tz0_f)THEN
c	   if(fiss.gt.-0.05d0)then
            if(abs(t50/aau(is,1)-tisst).le.sigmat*xxs.and.
...
c     &      .and.aau(1,3).gt.-0.15d0)then
              nscb(2)=nscb(2)+1
              sscb(nscb(2),2)=staru(is)
              do kkkkl=1,4
                sscpb(nscb(2),kkkkl,2)=aau(is,kkkkl)
              enddo
...
c     &      .and.aau(1,3).le.0.0d0)then
c	       nscb(2)=nscb(2)+1
c	       sscb(nscb(2),2)=staru(is)
c	       do kkkkl=1,4
c		 sscpb(nscb(2),kkkkl,2)=aau(is,kkkkl)
c	       enddo
...
c	    endif
	  ELSE
           if(abs(5040./aau(is,1)-tisst).le.sigmat*xxs.and.
     &	   abs(aau(is,2)-giss).le.sigmag*xxs.and.
     &	   abs(aau(is,3)-fiss).le.sigmaz*xxs)then
              nscb(2)=nscb(2)+1
              sscb(nscb(2),2)=staru(is)
              do kkkkl=1,4
...
c
	  IF(nbande(3).eq.0)THEN
          IF(aau(is,1).lt.tiss.and.aau(is,2).lt.giss.and.
     &      aau(is,3).ge.fiss)THEN
          IF(tiss.le.tz0_i.or.tiss.ge.tz0_f)THEN
c	   if(fiss.gt.-0.05d0)then
            if(abs(t50/aau(is,1)-tisst).le.sigmat*xxs.and.
...
c     &      .and.aau(1,3).gt.-0.15d0)then
              nscb(3)=nscb(3)+1
              sscb(nscb(3),3)=staru(is)
              do kkkkl=1,4
                sscpb(nscb(3),kkkkl,3)=aau(is,kkkkl)
              enddo
...
c     &      .and.aau(1,3).le.0.0d0)then
c	       nscb(3)=nscb(3)+1
c	       sscb(nscb(3),3)=staru(is)
c	       do kkkkl=1,4
c		 sscpb(nscb(3),kkkkl,3)=aau(is,kkkkl)
c	       enddo
...
c
	  IF(nbande(4).eq.0)THEN
          IF(aau(is,1).ge.tiss.and.aau(is,2).lt.giss.and.
     &      aau(is,3).ge.fiss)THEN
          IF(tiss.le.tz0_i.or.tiss.ge.tz0_f)THEN
c	   if(fiss.gt.-0.05d0)then
            if(abs(t50/aau(is,1)-tisst).le.sigmat*xxs.and.
...
c     &      .and.aau(1,3).gt.-0.15d0)then
              nscb(4)=nscb(4)+1
              sscb(nscb(4),4)=staru(is)
              do kkkkl=1,4
                sscpb(nscb(4),kkkkl,4)=aau(is,kkkkl)
              enddo
...
c     &      .and.aau(1,3).le.0.0d0)then
c	       nscb(4)=nscb(4)+1
c	       sscb(nscb(4),4)=staru(is)
c	       do kkkkl=1,4
c		 sscpb(nscb(4),kkkkl,4)=aau(is,kkkkl)
c	       enddo
...
c	    endif
	  ELSE
           if(abs(t50/aau(is,1)-tisst).le.sigmat*xxs.and.
     &	   abs(aau(is,2)-giss).le.sigmag*xxs.and.
     &	   abs(aau(is,3)-fiss).le.sigmaz)then
              nscb(4)=nscb(4)+1
              sscb(nscb(4),4)=staru(is)
              do kkkkl=1,4
                sscpb(nscb(4),kkkkl,4)=aau(is,kkkkl)
...
c     &      .and.aau(1,3).gt.-0.15d0)then
              nscb(5)=nscb(5)+1
              sscb(nscb(5),5)=staru(is)
              do kkkkl=1,4
                sscpb(nscb(5),kkkkl,5)=aau(is,kkkkl)
              enddo
...
c     &      .and.aau(1,3).le.0.0d0)then
c	       nscb(5)=nscb(5)+1
c	       sscb(nscb(5),5)=staru(is)
c	       do kkkkl=1,4
c		 sscpb(nscb(5),kkkkl,5)=aau(is,kkkkl)
c	       enddo
...
c     &      .and.aau(1,3).gt.-0.15d0)then
              nscb(6)=nscb(6)+1
              sscb(nscb(6),6)=staru(is)
              do kkkkl=1,4
                sscpb(nscb(6),kkkkl,6)=aau(is,kkkkl)
              enddo
...
c     &      .and.aau(1,3).le.0.0d0)then
c	       nscb(6)=nscb(6)+1
c	       sscb(nscb(6),6)=staru(is)
c	       do kkkkl=1,4
c		 sscpb(nscb(6),kkkkl,6)=aau(is,kkkkl)
c	       enddo
...
c     &      .and.aau(1,3).gt.-0.15d0)then
              nscb(7)=nscb(7)+1
              sscb(nscb(7),7)=staru(is)
              do kkkkl=1,4
                sscpb(nscb(7),kkkkl,7)=aau(is,kkkkl)
              enddo
...
c     &      .and.aau(1,3).le.0.0d0)then
c	       nscb(7)=nscb(7)+1
c	       sscb(nscb(7),7)=staru(is)
c	       do kkkkl=1,4
c		 sscpb(nscb(7),kkkkl,7)=aau(is,kkkkl)
c	       enddo
...
c     &      .and.aau(1,3).gt.-0.15d0)then
              nscb(8)=nscb(8)+1
              sscb(nscb(8),8)=staru(is)
              do kkkkl=1,4
                sscpb(nscb(8),kkkkl,8)=aau(is,kkkkl)
              enddo
...
c     &      .and.aau(1,3).le.0.0d0)then
c	       nscb(8)=nscb(8)+1
c	       sscb(nscb(8),8)=staru(is)
c	       do kkkkl=1,4
c		 sscpb(nscb(8),kkkkl,8)=aau(is,kkkkl)
c	       enddo
...
c OBTENCION DE ESTRELLA POR CUBO Y ESTRELLA FINAL NO NORMALIZADA
	DO lp=1,8 !numero de cubos
	  do jij=1,npxu !inicializo la estrella final cubo lp
...
229	FORMAT(2(I2,1x),A21,1x,f7.0,1x,f4.2,1(1x,f5.2),1x,f5.3,1x,G20.10)
	  ...
	  DO lp2=1,nsc(lp) !numero de estrellas en cubo lp
	   ...
	   do is=1,nstaru !estrellas libreria
	    if(ssc(lp2,lp).eq.staru(is))then
...
c      write(5555,229)lp,lp2,ssc(lp2,lp),t50/sscp(lp2,1,lp),
...
c DESCOMENTAR PARA ESCUPIR LO QUE HACE PARA CADA ESTRELLA
c      write(5555,*)'final=',lp,5040.d0/aloeci(1,lp),
...
c      write(5555,'(A3,1x,8(f11.9,1x))')'wei',(weigtc(lp),lp=1,8)
c      write(5555,'(A3,1x,8(f11.9,1x))')'WEF',(WE(lp),lp=1,8)
c      write(5555,*)'kolan=',kolan,' kolawn=',kolawn
c      write(5555,*)'nwe=',nwe,' nwef=',nwef
c      write(5555,*)'weifin=',weifin
c      if(weifin.le.1.0d0)WRITE(*,*)'WEIFIN LT ZERO'
c      if(weifin.le.1.0d0)write(5555,*)'WEIFIN LT ZERO'
c      if(nwef.eq.1) then
c      write(5555,'(A)')'ALMOST 1 BOX WAS USED FOR THE INTERPOLATION'
c      elseif(nwef.eq.2)then
c      write(5555,'(A)')'ALMOST 2 BOXES WERE USED FOR THE INTERPOLATION'
c      endif
c      write(5555,'(A3,1x,8(f11.9,1x))')'WEF',(WE(lp),lp=1,8)
C  Calcular volumenes calidad modelo  
      volpau=0.d0 !volumen escalado a sigma minimo
      do lp=1,8
       nbxstt=nbxstt+nsc(lp)
       volpau=volpau+sqrt((sigbox(lp,1)/(xs*ddt))**2.+
     & (sigbox(lp,2)/(xs*ddg))**2.+(sigbox(lp,3)/(xs*ddz))**2.)
      enddo
      xnbxsu=dble(nbxstt)
      volu10=0.d0 !volumen maximo aceptable (sigma=1.0)
      volu10=8.0d0*sqrt(((re10*ddtmax)/(xs*ddt))**2.+
     & ((re10*ddgmax)/(xs*ddg))**2.+((re10*ddzmax)/(xs*ddz))**2.) 
      volu05=0.d0 !volumen maximo aceptable (sigma=0.5)
      volu05=8.0d0*sqrt(((re05*ddtmax)/(xs*ddt))**2.+
     & ((re05*ddgmax)/(xs*ddg))**2.+((re05*ddzmax)/(xs*ddz))**2.) 
      volu15=0.d0 !volumen maximo aceptable (sigma=1.5)
      volu15=8.0d0*sqrt(((re15*ddtmax)/(xs*ddt))**2.+
