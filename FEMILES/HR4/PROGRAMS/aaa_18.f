...
cccccccccccccc Abundance ratios modelling approach ccccccccccccccccc
	...
	WRITE(*,*)'1 Varying abundance (stars based)?'
	WRITE(*,*)'2 Varying abundance & corrected [Mg/Fe](stars-based)?'
	WRITE(*,*)'9 SSP-based varying [Mg/Fe] & fully empirical(MILES)?'
	WRITE(*,*)'-----------------------------------------------------'
	iaba=0
	DeltFeH=0.0d0
	READ(*,*)iaba
...
478    format(A30,2(F7.3,1X))
	 ...
	 IF(amgfe.ne.0.0d0)THEN
	  write(*,*)'Coelho (1) or Allende-Prieto (0) gamma factor?'
	  read(*,*)ncoap
	  if(ncoap.eq.1)then
         gamma=0.75d0
...
c
5478	write(*,*)'MILES(m),MIUSC(i),NGSL(u),IRTF(k),ALL(a)?'
	read(*,*)miuk
	IF(miuk.eq.'m'.or.miuk.eq.'M')THEN
	  ...
	  if(iaba.eq.0)then
	    ...
	    mkout='mkdir ./OUT ./OUT/B ./OUT/C ./OUT/R ./OUT/M'
	  else
	    mkout='mkdir ./OUT /OUT/C ./OUT/M'
	  endif
	ELSEIF(miuk.eq.'i'.or.miuk.eq.'I')THEN
	  miuk='i'
	  dirout='ls ./OUT'
	  mkout='mkdir ./OUT ./OUT/I'
	ELSEIF(miuk.eq.'u'.or.miuk.eq.'U')THEN
	  miuk='u'
	  dirout='ls ./OUT'
	  mkout='mkdir ./OUT ./OUT/U'
	ELSEIF(miuk.eq.'k'.or.miuk.eq.'K')THEN
	  miuk='k'
	  dirout='ls ./OUT'
	  mkout='mkdir ./OUT ./OUT/K'
	ELSEIF(miuk.eq.'a'.or.miuk.eq.'A')THEN
	  miuk='a'
	  dirout='ls ./OUT'
	  if(iaba.eq.0)then
      mkout='mkdir ./OUT ./OUT/B ./OUT/C ./OUT/R ./OUT/M ./OUT/I ./OUT/U ./OUT/K'
        else
      mkout='mkdir ./OUT ./OUT/C ./OUT/M ./OUT/I ./OUT/U ./OUT/K'
	  endif
        write(*,*)mkout
	ELSE
	  goto 5478
...
31963	write(*,'(A)')'The options for the IMF shape are:'
      ...
      write(*,'(A)')'LaBARBERA = l' 
      write(*,'(A)')'u/b/k/r/c/f/x/l ?'
c	write(*,*)'LB IMF, others ignored'
 	read(*,'(A1)')shape
c      shape='l'
	if(shape.eq.'b') then
...
c
	 ...
	 CLOSE(40)
	ENDIF
	IF(miuk.eq.'i'.or.miuk.eq.'a')THEN
	 OPEN(49,FILE='./OUT/out_u_I',STATUS='OLD',ERR=6648)
	 CLOSE(49,STATUS='DELETE')
6648	 OPEN(49,IOSTAT=IOS,FILE='./OUT/out_u_I',STATUS='NEW')
	 OPEN(50,FILE='./OUT/out_ef_I',STATUS='OLD',ERR=6649)
	 CLOSE(50,STATUS='DELETE')
6649	 OPEN(50,IOSTAT=IOS,FILE='./OUT/out_ef_I',STATUS='NEW')
	 OPEN(399,FILE='./OUT/out_qua_I',STATUS='OLD',ERR=6650)
	 CLOSE(399,STATUS='DELETE')
6650	 OPEN(399,IOSTAT=IOS,FILE='./OUT/out_qua_I',STATUS='NEW')
c
       CALL a_MIUSC
c
	 CLOSE(49)
