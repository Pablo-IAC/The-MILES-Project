c*******SUBRUTINA DE Hdeltaf************************************************
c        SUBROUTINE hdeltf(nra,i,W,stars)
        SUBROUTINE hdeltf(i,W,stars)
...
c        feh(i)=fez(i)+dlog10(ra(nra,1)/ra(1,1))
	spend=(4101.625-3600.)/(4400.-3600.)
	W=0.0
	stars=0.0
	if(jotai(i).eq.0)goto 4041
	DO j=1,jotai(i)
	...
	ZMAG=-17.229+92.930*tetas-85.4316*tetas*tetas-0.5143*feh(i)
	  elseif(R(i,j,3).ge.3.7076.and.R(i,j,3).le.3.826075)then
	ZMAG=47.273-86.425*tetas+38.573*tetas*tetas-0.631*tetas*feh(i)
	  elseif(R(i,j,3).lt.3.7076)then
		if(R(i,j,3).lt.3.5798.and.lc.eq.3)then
	ZMAG=-8.528+4.334e-3*at-6.195e-7*at*at
	  	elseif(R(i,j,3).lt.3.6335.and.lc.eq.5)then
	ZMAG=-14.584+9.716e-3*at-1.528e-6*at*at
	  	else
	ZMAG=-1.0345-10.347*t-1.342*feh(i)+36.251*t*t*R(i,j,4)
	  	endif
	  else
	  	goto 5041
	  endif
