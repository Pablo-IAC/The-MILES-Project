...
c CaT y MILES para una terna de parametros atmosfericos. 
      SUBROUTINE hrsl_IRTF(k,wflujk,al99k,fja99k,qum,qu10,qu05,qu15)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
c IRTF
      PARAMETER (npxk=15387) !IRTF
      DIMENSION ssppk(npxk,2),wflujk(npxk,2) !IRTF
...
ccccccccccccccccccccccccc
	 ...
	 DO l=1,npxk !IRTF
	  ssppk(l,2)=ssppk(l,2)/fkmil
	   ...
	   if(imam(k).gt.0)then
	    do mma=1,imam(k)
	     wflujk(l,2)=wflujk(l,2)+ssppk(l,2)*R140(k,mma)*flujk
	    enddo
	   endif
	   wflujk(l,2)=wflujk(l,2)+ssppk(l,2)*R(k,1,14)*flujk
	  else
	   wflujk(l,2)=wflujk(l,2)+ssppk(l,2)*R(k,m,14)*flujk
	  endif
	 ENDDO
       if(m.eq.1)then
	   if(imam(k).gt.0)then
