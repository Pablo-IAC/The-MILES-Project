      SUBROUTINE sigmam_IRTF(tiss,giss,fiss,ala,aloec)
CC Version 29-February-2024.
C------------------------------------------------------------------------------
C 	Copyright A. Vazdekis 
C 	Instituto de Astrofisica de Canarias
C 	E-mail: vazdekis@iac.es
C------------------------------------------------------------------------------
c This program computes a star spectrum for a set of atmospheric parameters
c in the wavelength range covered by the IRTF library
C------------------------------------------------------------------------------
C INPUT FILES:
...
c IRTF
c      PARAMETER (npxk=15387) !IRTF
      PARAMETER (npxk=21426) !IRTF #pixels spectra extended 7150 disp2A
      PARAMETER (nstk=400) !IRTF
      DIMENSION sscb(nstk,8),sscpb(nstk,4,8),ssc(nstk,8),sscp(nstk,4,8)
      DIMENSION ala(npxk,2),weigc(nstk,8),cubstc(npxk,8)
c
...
c
      ...
      ultra0=1.0d-12
      t50=5040.0d0
c	tz0_i=3750.0d0 !temp mas baja para la que distinguimos[M/H]
c	tz0_f=9000.0d0 !temp mas alta para la que distinguimos [M/H]
c	tz0_i=4000.0d0 !temp mas baja para la que distinguimos[M/H]
c	tz0_f=6000.0d0 !temp mas alta para la que distinguimos [M/H]
      tz0_i=3250.0d0 !temp mas baja para la que distinguimos[M/H]
      tz0_f=7000.0d0 !temp mas alta para la que distinguimos [M/H]
      blc1=18500.0d0 !lambda inicial filtro K donde supera 0.
      blc2=25500.0d0 !lambda final filtro K donde supera 0.
c	blc1=19400.0d0 !lambda inicial filtro K donde supera 0.
c       blc2=24800.0d0 !lambda final filtro K donde supera 0.
c	ddt0=0.009d0
c	tlinmi=60.0d0
c	ddt0M=0.1696d0
c	tlinma=3355.0d0
...
c	ddzmax=0.408d0
	ddg=0.11d0
c	ddz=0.05d0
c	ddgmax=0.59d0 ;values at work at 2025
c	ddzmax=0.30d0 ;values at work at 2025
	ddgmax=0.512d0
	ddzmax=0.408d0
	do isbox=1,8
	...
	enddo
c	DMA=300.0d0 !densidad limite (0.9974 frac.cum.) NON EXTENDED
c	DMA=400.0d0 !densidad limite (0.9974 frac.cum.) ONLY NEW STARS
	DMA=143.0d0 !densidad limite (0.9974 frac.cum.) EXTENDED
c	DMA=700.0d0 !densidad limite (0.9974 frac.cum.) EXTENDED
	...
	snamax=100.0d0 !0.970408 CAT(367)CAT 2.5sigma,142.,254.,367.
	snamin=25.0d0 !0.03 CAT(22) !2sigma,13.,17.5,22. son 155*<=22.0
c
	xnumno=0.0d0 !numero pixels en banda normalizacion
	do jij=1,npxk
	  ala(jij,1)=starks(1,jij,1)
...
c     &  abs(aak(l,2)-giss).le.dddg.and.aak(1,3).gt.-0.15d0)then
     &  abs(aak(l,2)-giss).le.dddg)then
     	      starsi=starsi+1.0d0
	endif
c	    else
c	if(abs(t50/aak(l,1)-tisst).le.dddt.and.
c     &  abs(aak(l,2)-giss).le.dddg.and.aak(1,3).le.0.0d0)then
...
c BUSQUEDA ESTRELLAS DE ACUERDO A ESTAS SIGMAS PARA 8 CUBOS
	...
	rbandi=0.0d0 !bandera para detener proceso si hay * en 8cubos
	 ...
	 xxs=xs+rntt*fxs !factor que multiplica a sigma
         DO is=1,nstark !de las estrellas de la libreria
c       __________________________________________________
c	 if (is.ne.nist) then !exclusion estrella a ajustar
c       __________________________________________________
	  IF(nbande(1).eq.0)THEN
          IF(aak(is,1).ge.tiss.and.aak(is,2).ge.giss.and.
     &    aak(is,3).ge.fiss)THEN
          IF(tiss.le.tz0_i.or.tiss.ge.tz0_f)THEN
c	   if(fiss.gt.-0.05d0)then
            if(abs(t50/aak(is,1)-tisst).le.sigmat*xxs.and.
     &      abs(aak(is,2)-giss).le.sigmag*xxs)then
c     &      .and.aak(1,3).gt.-0.15d0)then
              nscb(1)=nscb(1)+1
              sscb(nscb(1),1)=stark(is)
              do kkkkl=1,4
                sscpb(nscb(1),kkkkl,1)=aak(is,kkkkl)
              enddo
...
c	   else
c            if(abs(t50/aak(is,1)-tisst).le.sigmat*xxs.and.
c     &      abs(aak(is,2)-giss).le.sigmag*xxs
c     &      .and.aak(1,3).le.0.0d0)then
c              nscb(1)=nscb(1)+1
c              sscb(nscb(1),1)=stark(is)
c              do kkkkl=1,4
c                sscpb(nscb(1),kkkkl,1)=aak(is,kkkkl)
c              enddo
...
c	   endif
	  ELSE
           if(abs(t50/aak(is,1)-tisst).le.sigmat*xxs.
     &	   and.abs(aak(is,2)-giss).le.sigmag*xxs.and.
     &	   abs(aak(is,3)-fiss).le.sigmaz*xxs)then
              nscb(1)=nscb(1)+1
              sscb(nscb(1),1)=stark(is)
              do kkkkl=1,4
                sscpb(nscb(1),kkkkl,1)=aak(is,kkkkl)
...
c
	  IF(nbande(2).eq.0)THEN
          IF(aak(is,1).lt.tiss.and.aak(is,2).ge.giss.and.
     &      aak(is,3).ge.fiss)THEN
          IF(tiss.le.tz0_i.or.tiss.ge.tz0_f)THEN
c	   if(fiss.gt.-0.05d0)then
            if(abs(t50/aak(is,1)-tisst).le.sigmat*xxs.and.
...
c     &      .and.aak(1,3).gt.-0.15d0)then
              nscb(2)=nscb(2)+1
              sscb(nscb(2),2)=stark(is)
              do kkkkl=1,4
                sscpb(nscb(2),kkkkl,2)=aak(is,kkkkl)
              enddo
...
c     &      .and.aak(1,3).le.0.0d0)then
c	       nscb(2)=nscb(2)+1
c	       sscb(nscb(2),2)=stark(is)
c	       do kkkkl=1,4
c		 sscpb(nscb(2),kkkkl,2)=aak(is,kkkkl)
c	       enddo
...
c	    endif
	  ELSE
           if(abs(5040./aak(is,1)-tisst).le.sigmat*xxs.and.
     &	   abs(aak(is,2)-giss).le.sigmag*xxs.and.
     &	   abs(aak(is,3)-fiss).le.sigmaz*xxs)then
              nscb(2)=nscb(2)+1
              sscb(nscb(2),2)=stark(is)
              do kkkkl=1,4
...
c
	  IF(nbande(3).eq.0)THEN
          IF(aak(is,1).lt.tiss.and.aak(is,2).lt.giss.and.
     &      aak(is,3).ge.fiss)THEN
          IF(tiss.le.tz0_i.or.tiss.ge.tz0_f)THEN
c	   if(fiss.gt.-0.05d0)then
            if(abs(t50/aak(is,1)-tisst).le.sigmat*xxs.and.
...
c     &      .and.aak(1,3).gt.-0.15d0)then
              nscb(3)=nscb(3)+1
              sscb(nscb(3),3)=stark(is)
              do kkkkl=1,4
                sscpb(nscb(3),kkkkl,3)=aak(is,kkkkl)
              enddo
...
c     &      .and.aak(1,3).le.0.0d0)then
c	       nscb(3)=nscb(3)+1
c	       sscb(nscb(3),3)=stark(is)
c	       do kkkkl=1,4
c		 sscpb(nscb(3),kkkkl,3)=aak(is,kkkkl)
c	       enddo
...
c
	  IF(nbande(4).eq.0)THEN
          IF(aak(is,1).ge.tiss.and.aak(is,2).lt.giss.and.
     &      aak(is,3).ge.fiss)THEN
          IF(tiss.le.tz0_i.or.tiss.ge.tz0_f)THEN
c	   if(fiss.gt.-0.05d0)then
            if(abs(t50/aak(is,1)-tisst).le.sigmat*xxs.and.
...
c     &      .and.aak(1,3).gt.-0.15d0)then
              nscb(4)=nscb(4)+1
              sscb(nscb(4),4)=stark(is)
              do kkkkl=1,4
                sscpb(nscb(4),kkkkl,4)=aak(is,kkkkl)
              enddo
...
c     &      .and.aak(1,3).le.0.0d0)then
c	       nscb(4)=nscb(4)+1
c	       sscb(nscb(4),4)=stark(is)
c	       do kkkkl=1,4
c		 sscpb(nscb(4),kkkkl,4)=aak(is,kkkkl)
c	       enddo
...
c	    endif
	  ELSE
           if(abs(t50/aak(is,1)-tisst).le.sigmat*xxs.and.
     &	   abs(aak(is,2)-giss).le.sigmag*xxs.and.
     &	   abs(aak(is,3)-fiss).le.sigmaz)then
              nscb(4)=nscb(4)+1
              sscb(nscb(4),4)=stark(is)
              do kkkkl=1,4
                sscpb(nscb(4),kkkkl,4)=aak(is,kkkkl)
...
c     &      .and.aak(1,3).gt.-0.15d0)then
              nscb(5)=nscb(5)+1
              sscb(nscb(5),5)=stark(is)
              do kkkkl=1,4
                sscpb(nscb(5),kkkkl,5)=aak(is,kkkkl)
              enddo
...
c     &      .and.aak(1,3).le.0.0d0)then
c	       nscb(5)=nscb(5)+1
c	       sscb(nscb(5),5)=stark(is)
c	       do kkkkl=1,4
c		 sscpb(nscb(5),kkkkl,5)=aak(is,kkkkl)
c	       enddo
...
c     &      .and.aak(1,3).gt.-0.15d0)then
              nscb(6)=nscb(6)+1
              sscb(nscb(6),6)=stark(is)
              do kkkkl=1,4
                sscpb(nscb(6),kkkkl,6)=aak(is,kkkkl)
              enddo
...
c     &      .and.aak(1,3).le.0.0d0)then
c	       nscb(6)=nscb(6)+1
c	       sscb(nscb(6),6)=stark(is)
c	       do kkkkl=1,4
c		 sscpb(nscb(6),kkkkl,6)=aak(is,kkkkl)
c	       enddo
...
c     &      .and.aak(1,3).gt.-0.15d0)then
              nscb(7)=nscb(7)+1
              sscb(nscb(7),7)=stark(is)
              do kkkkl=1,4
                sscpb(nscb(7),kkkkl,7)=aak(is,kkkkl)
              enddo
...
c     &      .and.aak(1,3).le.0.0d0)then
c	       nscb(7)=nscb(7)+1
c	       sscb(nscb(7),7)=stark(is)
c	       do kkkkl=1,4
c		 sscpb(nscb(7),kkkkl,7)=aak(is,kkkkl)
c	       enddo
...
c     &      .and.aak(1,3).gt.-0.15d0)then
              nscb(8)=nscb(8)+1
              sscb(nscb(8),8)=stark(is)
              do kkkkl=1,4
                sscpb(nscb(8),kkkkl,8)=aak(is,kkkkl)
              enddo
...
c     &      .and.aak(1,3).le.0.0d0)then
c	       nscb(8)=nscb(8)+1
c	       sscb(nscb(8),8)=stark(is)
c	       do kkkkl=1,4
c		 sscpb(nscb(8),kkkkl,8)=aak(is,kkkkl)
c	       enddo
...
c OBTENCION DE ESTRELLA POR CUBO Y ESTRELLA FINAL NO NORMALIZADA
	DO lp=1,8 !numero de cubos
	  do jij=1,npxk !inicializo la estrella final cubo lp
...
229	FORMAT(2(I2,1x),A21,1x,f7.0,1x,f4.2,1(1x,f5.2),1x,f5.3,1x,G20.10)
	  ...
	  DO lp2=1,nsc(lp) !numero de estrellas en cubo lp
	   ...
	   do is=1,nstark !estrellas libreria
	    if(ssc(lp2,lp).eq.stark(is))then
...
c      write(55555,229)lp,lp2,ssc(lp2,lp),t50/sscp(lp2,1,lp),
...
c DESCOMENTAR PARA ESCUPIR LO QUE HACE PARA CADA ESTRELLA
c      write(55555,*)'final=',lp,5040.d0/aloeci(1,lp),
...
c      write(55555,'(A3,1x,8(f11.9,1x))')'wei',(weigtc(lp),lp=1,8)
c      write(55555,'(A3,1x,8(f11.9,1x))')'WEF',(WE(lp),lp=1,8)
c      write(55555,*)'kolan=',kolan,' kolawn=',kolawn
c      write(55555,*)'nwe=',nwe,' nwef=',nwef
c      write(55555,*)'weifin=',weifin
      if(weifin.le.1.0d0)WRITE(*,*)'WEIFIN IRTF LT ZERO'
c      if(weifin.le.1.0d0)write(55555,*)'WEIFIN LT ZERO'
      if(nwef.eq.1) then
c      write(55555,'(A)')'ALMOST 1 BOX WAS USED FOR THE INTERPOLATION'
      elseif(nwef.eq.2)then
c      write(55555,'(A)')'ALMOST 2 BOXES WERE USED FOR THE INTERPOLATION'
      endif
c      write(55555,'(A3,1x,8(f11.9,1x))')'WEF',(WE(lp),lp=1,8)
C  Calcular volumenes calidad modelo  
      volpak=0.d0 !volumen escalado a sigma minimo
      do lp=1,8
       nbxstt=nbxstt+nsc(lp)
       volpak=volpak+sqrt((sigbox(lp,1)/(xs*ddt))**2.d0+
     &(sigbox(lp,2)/(xs*ddg))**2.d0+(sigbox(lp,3)/(xs*ddz))**2.d0)
      enddo
      xnbxsk=dble(nbxstt)
      volk10=0.d0 !volumen maximo aceptable (sigma=1.0)
      volk10=8.d0*sqrt(((re10*ddtmax)/(xs*ddt))**2.d0+
     &((re10*ddgmax)/(xs*ddg))**2.d0+((re10*ddzmax)/(xs*ddz))**2.d0) 
      volk05=0.d0 !volumen maximo aceptable (sigma=0.5)
      volk05=8.0d0*sqrt(((re05*ddtmax)/(xs*ddt))**2.d0+
     &((re05*ddgmax)/(xs*ddg))**2.d0+((re05*ddzmax)/(xs*ddz))**2.d0) 
      volk15=0.d0 !volumen maximo aceptable (sigma=1.5)
      volk15=8.d0*sqrt(((re15*ddtmax)/(xs*ddt))**2.d0+
     &((re15*ddgmax)/(xs*ddg))**2.d0+((re15*ddzmax)/(xs*ddz))**2.d0)
      RETURN
      END	
