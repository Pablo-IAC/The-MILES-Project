      SUBROUTINE sigmam(tiss,giss,fiss,ala,aloec)
CC Version 21-September-2004.
C------------------------------------------------------------------------------
...
c      COMMON/hrcat/aam(nstm,4),nstarm
c      COMMON/clstdc/starm(nstm)
c      COMMON/exclu/nist
      DIMENSION ssc(nstm,8),sscp(nstm,4,8),nsc(8),nbande(8)
      DIMENSION sscb(nstm,8),sscpb(nstm,4,8),nscb(8)
...
c	    if(fiss.gt.-0.05d0)then
	if(abs(t50/aam(l,1)-tisst).le.dddt.and.
c     &  abs(aam(l,2)-giss).le.dddg.and.aam(1,3).gt.-0.15d0)then
     &  abs(aam(l,2)-giss).le.dddg)then
     	      starsi=starsi+1.0d0
	endif
c	    else
c	if(abs(t50/aam(l,1)-tisst).le.dddt.and.
c     &  abs(aam(l,2)-giss).le.dddg.and.aam(1,3).le.0.0d0)then
c     	      starsi=starsi+1.0d0
c	endif
c	    endif
	   ELSE
	    if(abs(t50/aam(l,1)-tisst).le.dddt
     &       .and.abs(aam(l,2)-giss).le.dddg
     &       .and.abs(aam(l,3)-fiss).le.dddz)then
     	     starsi=starsi+1.0d0
	    endif
	   ENDIF
	  enddo
...
c       __________________________________________________
	  IF(nbande(1).eq.0)THEN
          IF(aam(is,1).ge.tiss.and.aam(is,2).ge.giss.and.
     &    aam(is,3).ge.fiss)THEN
          IF(tiss.le.tz0_i.or.tiss.ge.tz0_f)THEN
c	   if(fiss.gt.-0.05d0)then
            if(abs(t50/aam(is,1)-tisst).le.sigmat*xxs.and.
     &      abs(aam(is,2)-giss).le.sigmag*xxs)then
c     &      .and.aam(1,3).gt.-0.15d0)then
              nscb(1)=nscb(1)+1
              sscb(nscb(1),1)=starm(is)
              do kkkkl=1,4
                sscpb(nscb(1),kkkkl,1)=aam(is,kkkkl)
              enddo
...
c	   else
c            if(abs(t50/aam(is,1)-tisst).le.sigmat*xxs.and.
c     &      abs(aam(is,2)-giss).le.sigmag*xxs
c     &      .and.aam(1,3).le.0.0d0)then
c              nscb(1)=nscb(1)+1
c              sscb(nscb(1),1)=starm(is)
c              do kkkkl=1,4
c                sscpb(nscb(1),kkkkl,1)=aam(is,kkkkl)
c              enddo
...
c	   endif
	  ELSE
           if(abs(t50/aam(is,1)-tisst).le.sigmat*xxs.
     &	   and.abs(aam(is,2)-giss).le.sigmag*xxs.and.
     &	   abs(aam(is,3)-fiss).le.sigmaz*xxs)then
              nscb(1)=nscb(1)+1
              sscb(nscb(1),1)=starm(is)
              do kkkkl=1,4
                sscpb(nscb(1),kkkkl,1)=aam(is,kkkkl)
...
c
	  IF(nbande(2).eq.0)THEN
          IF(aam(is,1).lt.tiss.and.aam(is,2).ge.giss.and.
     &      aam(is,3).ge.fiss)THEN
          IF(tiss.le.tz0_i.or.tiss.ge.tz0_f)THEN
c	   if(fiss.gt.-0.05d0)then
            if(abs(t50/aam(is,1)-tisst).le.sigmat*xxs.and.
...
c     &      .and.aam(1,3).gt.-0.15d0)then
              ...
              do kkkkl=1,4
                sscpb(nscb(2),kkkkl,2)=aam(is,kkkkl)
              enddo
...
c	       do kkkkl=1,4
c		 sscpb(nscb(2),kkkkl,2)=aam(is,kkkkl)
c	       enddo
...
c	    endif
	  ELSE
           if(abs(5040./aam(is,1)-tisst).le.sigmat*xxs.and.
     &	   abs(aam(is,2)-giss).le.sigmag*xxs.and.
     &	   abs(aam(is,3)-fiss).le.sigmaz*xxs)then
              nscb(2)=nscb(2)+1
              sscb(nscb(2),2)=starm(is)
              do kkkkl=1,4
...
c
	  IF(nbande(3).eq.0)THEN
          IF(aam(is,1).lt.tiss.and.aam(is,2).lt.giss.and.
     &      aam(is,3).ge.fiss)THEN
          IF(tiss.le.tz0_i.or.tiss.ge.tz0_f)THEN
c	   if(fiss.gt.-0.05d0)then
            if(abs(t50/aam(is,1)-tisst).le.sigmat*xxs.and.
...
c     &      .and.aam(1,3).gt.-0.15d0)then
              ...
              do kkkkl=1,4
                sscpb(nscb(3),kkkkl,3)=aam(is,kkkkl)
              enddo
...
c	       do kkkkl=1,4
c		 sscpb(nscb(3),kkkkl,3)=aam(is,kkkkl)
c	       enddo
...
c
	  IF(nbande(4).eq.0)THEN
          IF(aam(is,1).ge.tiss.and.aam(is,2).lt.giss.and.
     &      aam(is,3).ge.fiss)THEN
          IF(tiss.le.tz0_i.or.tiss.ge.tz0_f)THEN
c	   if(fiss.gt.-0.05d0)then
            if(abs(t50/aam(is,1)-tisst).le.sigmat*xxs.and.
...
c     &      .and.aam(1,3).gt.-0.15d0)then
              ...
              do kkkkl=1,4
                sscpb(nscb(4),kkkkl,4)=aam(is,kkkkl)
              enddo
...
c	       do kkkkl=1,4
c		 sscpb(nscb(4),kkkkl,4)=aam(is,kkkkl)
c	       enddo
...
c	    endif
	  ELSE
           if(abs(t50/aam(is,1)-tisst).le.sigmat*xxs.and.
     &	   abs(aam(is,2)-giss).le.sigmag*xxs.and.
     &	   abs(aam(is,3)-fiss).le.sigmaz)then
              nscb(4)=nscb(4)+1
              sscb(nscb(4),4)=starm(is)
              do kkkkl=1,4
                sscpb(nscb(4),kkkkl,4)=aam(is,kkkkl)
...
c     &      .and.aam(1,3).gt.-0.15d0)then
              ...
              do kkkkl=1,4
                sscpb(nscb(5),kkkkl,5)=aam(is,kkkkl)
              enddo
...
c	       do kkkkl=1,4
c		 sscpb(nscb(5),kkkkl,5)=aam(is,kkkkl)
c	       enddo
...
c     &      .and.aam(1,3).gt.-0.15d0)then
              ...
              do kkkkl=1,4
                sscpb(nscb(6),kkkkl,6)=aam(is,kkkkl)
              enddo
...
c	       do kkkkl=1,4
c		 sscpb(nscb(6),kkkkl,6)=aam(is,kkkkl)
c	       enddo
...
c     &      .and.aam(1,3).gt.-0.15d0)then
              ...
              do kkkkl=1,4
                sscpb(nscb(7),kkkkl,7)=aam(is,kkkkl)
              enddo
...
c	       do kkkkl=1,4
c		 sscpb(nscb(7),kkkkl,7)=aam(is,kkkkl)
c	       enddo
...
c     &      .and.aam(1,3).gt.-0.15d0)then
              ...
              do kkkkl=1,4
                sscpb(nscb(8),kkkkl,8)=aam(is,kkkkl)
              enddo
...
c	       do kkkkl=1,4
c		 sscpb(nscb(8),kkkkl,8)=aam(is,kkkkl)
c	       enddo
...
c para evitar numeros negativos en el espectro estelar debido a baja S/R
	     OPEN(99,file=starm(is),status='old')
	     atonmi=9.e19
	     do jij=1,npxm
	read(99,*)atonii,aton
	if(aton.gt.0.0d0.and.aton.lt.atonmi) atonmi=aton	     
	     enddo
	     close(99)
...
c DESCOMENTAR PARA ESCUPIR LO QUE HACE PARA CADA ESTRELLA
      WRITE(55,229)lp,lp2,ssc(lp2,lp),t50/sscp(lp2,1,lp),
     &(sscp(lp2,la,lp),la=2,3),snaton,weigc(lp2,lp)
	  ENDDO
...
c DESCOMENTAR PARA ESCUPIR LO QUE HACE PARA CADA ESTRELLA
      WRITE(55,*)'final=',lp,5040.d0/aloeci(1,lp),
     &aloeci(2,lp),aloeci(3,lp),snat(lp),nsc(lp),weigtc(lp)
	  do jij=1,npxm
...
c fichero para testear lo que hace:
      WRITE(55,'(A3,1x,8(f11.9,1x))')'wei',(weigtc(lp),lp=1,8)
      WRITE(55,'(A3,1x,8(f11.9,1x))')'WEF',(WE(lp),lp=1,8)
      WRITE(55,*)'kolan=',kolan,' kolawn=',kolawn
      WRITE(55,*)'nwe=',nwe,' nwef=',nwef
      WRITE(55,*)'weifin=',weifin
      if(weifin.le.1.0d0)WRITE(*,*)'WEIFIN LT ZERO'
      if(weifin.le.1.0d0)WRITE(55,*)'WEIFIN LT ZERO'
      if(nwef.eq.1) then
      WRITE(55,'(A)')'ALMOST 1 BOX WAS USED FOR THE INTERPOLATION'
...
c      WRITE(*,'(A)')'ALMOST 1 BOX WAS USED FOR THE INTERPOLATION'
      ...
      WRITE(55,'(A)')'ALMOST 2 BOXES WERE USED FOR THE INTERPOLATION'
