...
c*******SUBRUTINA HRSL************************************************
      SUBROUTINE hrsl_alpha(k,wflujb,wflujv,wflujc,wflujm,al99,fjal99,
     ...
     &fluxUm,fluxUi,fluxU,fluUms,fluUis,fluUs,fluUis004)
      ...
      DIMENSION ssppm(npxm,2) !miles
	DIMENSION ssppml(npxm,2) !miles
	DIMENSION ssppmh(npxm,2) !miles
      DIMENSION wflujm(npxm,2) !miles
...
c      call cotojo(giss,vrkc,vikc,vij)
       call xflabs(uvbus,biss,viss,vikc,vk,flujb,flujv,
     &flujcj,fluju,flujk)
...
c Preparando la dimension Mg/Fe
       ...
       whig=0.5d0
      IF(iaba.eq.9)THEN
       call sigmam_alpha(aamlo,tiss,giss,fiss,ssppml,alml,sigtgl,volpl)
       call sigmam_alpha(aamhi,tiss,giss,fiss,ssppmh,almh,sigtgh,volph)
