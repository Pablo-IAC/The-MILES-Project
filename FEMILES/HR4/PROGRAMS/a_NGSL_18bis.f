...
c*******COMIENZO DEL BUCLE DE LA SSPo**********************************
	  DO izo=1,nzold
	   ...
	   DO iao=1,ntold
	    ageo=ttold(iao)
          call STU_NGSL(izold(izo),ageo,BETAo,realeo,ZMISOo,
     &wwffuo,a9uo,f9mo,qumo,qu10o,qu05o,qu15o)
	    attao=realeo
...
c*******COMIENZO DEL BUCLE DE LA SSPy**********************************
     ...
     &abs(pizyng(izyng(izy)))
		...
		agey=ttyng(iay)
	      ...
	      IF(alpha.ne.1.0d0)THEN
	       call STU_NGSL(izyng(izy),agey,BETAy,realey,ZMISOy,
     &wwffuy,a9uy,f9my,qumy,qu10y,qu05y,qu15y)
	       attao=realey
	       if(attao.lt.10.0d0)then
	        write(chato(1:8),'(A,A,f6.4)')'T','0',attao
	       else
