...
c CaT y MILES para una terna de parametros atmosfericos. 
      SUBROUTINE hrsl_NGSL(k,wfluju,al99u,fja99u,qum,qu10,qu05,qu15)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
c NGSL
      PARAMETER (npxu=6192) !NGSL
      DIMENSION ssppu(npxu,2),wfluju(npxu,2) !NGSL
...
ccccccccccccccccccccccccc
	...
	DO l=1,npxu !NGSL
	 ssppu(l,2)=ssppu(l,2)/fvmil
	  ...
	  if(imam(k).gt.0)then
	   do mma=1,imam(k)
	    wfluju(l,2)=wfluju(l,2)+ssppu(l,2)*R140(k,mma)*flujv
	   enddo
	  endif
	  wfluju(l,2)=wfluju(l,2)+ssppu(l,2)*R(k,1,14)*flujv
	 else
	  wfluju(l,2)=wfluju(l,2)+ssppu(l,2)*R(k,m,14)*flujv
	 endif
	ENDDO
      if(m.eq.1)then
	  if(imam(k).gt.0)then
