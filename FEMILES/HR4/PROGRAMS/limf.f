...
c    double precision sson(4)
      ...
      COMMON/IGIMF/yIGMFm(9999),yIGMFn(9999),igm !also in a_*.f
...
c         fimm=(xxmasa/bicl)**(-0.3d0)*(bicp**(-ZMU+1.0d0)/bicl) !if lowest segment decreases and not flat
     ...
     &  *1.0d0+sson(4)/xxmasa
      ...
      ELSEIF(shape.eq.'t')THEN
       if(xxmasa.lt.xxmab0)then
        fimm=(xxmasa/xxmab0)**(-alfb0)
       elseif(xxmasa.ge.xxmab0.and.xxmasa.lt.xxmab1)then
        fimm=(xxmasa/xxmab0)**(-alfb1)
       else
        fimm=(xxmab1/xxmab0)**(-alfb1)*(xxmasa/xxmab1)**(-alfb2)
       endif	 
     ...
     & (xxmasa/xxmar2)**(-alfr3)
       ...
       ELSEIF(shape.eq.'g')THEN
        call hunt(yIGMFm,igm,xxmasa,k1)
        if(k1.lt.2)then
         call polint(yIGMFm(k1),yIGMFn(k1),2,xxmasa,fimm,dy)
        elseif(k1.gt.igm-1)then
         call polint(yIGMFm(k1-1),yIGMFn(k1-1),2,xxmasa,fimm,dy)
        else
         call polint(yIGMFm(k1-1),yIGMFn(k1-1),3,xxmasa,fimm,dy)
        endif
...
c       fimm=exp(-(((dlog10(xxmasa)-c008)**2.d0)/csigma))
        ...
        fimm=xxmasa**(-2.3d0)
c        fimm=xxmasa**(-ZMU) !to allow varying the slope of the upper segment
       endif
      ELSE
       fimm=xxmasa**(-ZMU)
      ENDIF
