...
c IRTF
c      PARAMETER (npxk=15387) !IRTF #pixels in the spectra
      PARAMETER (npxk=21426) !IRTF #pixels spectra extended 7150 disp2A
      PARAMETER (nstk=400) !IRTF #stars in the library
      DIMENSION wfk(npxk,2),wwffko(npxk,2),wwffky(npxk,2) !IRTF
      DIMENSION wfks(npxk,2),wwffkso(npxk,2),wwffksy(npxk,2) !IRTF
	 ...
	 elseif(shape.eq.'u')then
	  ...
	  chatom='./OUT/K/Kun'//chato !IRTF
         elseif(shape.eq.'g')then
          ...
          chatom='./OUT/K/Kg'//chato(1:5)
	 elseif(shape.eq.'t')then
	  write(chato(1:4),'(f4.2)')ZMU-1.0d0
	  chatom='./OUT/K/Kba'//chato !IRTF
...
c     ==================== END IGIMF OPTION ===========================
	   DO iao=1,ntold
	    ageo=ttold(iao)
          call STU_IRTF(izold(izo),ageo,BETAo,realeo,ZMISOo,wwffko,
     &wwffkso,a9ko,f9mo,qumo,qu10o,qu05o,qu15o)
	    attao=realeo
...
c*******COMIENZO DEL BUCLE DE LA SSPy**********************************
     ...
     &abs(pizyng(izyng(izy)))
		...
		agey=ttyng(iay)
	      ...
	      IF(alpha.ne.1.0d0)THEN
	       call STU_IRTF(izyng(izy),agey,BETAy,realey,ZMISOy,wwffky,
     &wwffksy,a9ky,f9my,qumy,qu10y,qu05y,qu15y)
	       attao=realey
	       if(attao.lt.10.0d0)then
	        write(chato(1:8),'(A,A,f6.4)')'T','0',attao
	       else
...
c           Parametros calidad IRTF
     ...
     &BETAy*wwffky(io,2)
             wfks(io,2)=alpha*BETAo*wwffkso(io,2)+(1.d0-alpha)*
     &BETAy*wwffksy(io,2)
             wfks(io,1)=wfk(io,1)
            enddo
cccccccc
...
8989	      OPEN(77,IOSTAT=IOS,FILE=chatoms,STATUS='NEW')
	      ...
	      enddo
	      do lup=1,npxk
c		 write(77,*)wfks(lup,1),wfks(lup,2)/wfk(lup,2)
		 write(77,*)wfks(lup,1),wfks(lup,2)
	      enddo
	      close(77)
