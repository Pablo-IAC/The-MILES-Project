...
c CaT y MILES para una terna de parametros atmosfericos. 
      SUBROUTINE hrsl_XSL_276A(k,wfluji,wflujis,al99i,fja99i,qum,qu10,
     &qu05,qu15)
cc      SUBROUTINE hrsl_XSL(k,wfluji,wflujis,al99i,fja99i,qum,qu10,
cc     &qu05,qu15,fluxUm,fluxUi,fluxU,fluUms,fluUis,fluUs,fluUis004)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
c XSL
      PARAMETER (npxi=23578) !XSL
c
      DIMENSION ssppi(npxi,2),wfluji(npxi,2),wflujis(npxi,2)!XSL
...
c solo para sigmam.f
      ...
      fiss=fez(k)-DeltFeH
ccccccccccccccccccccccccc
      qum=0.0d0 !parametro calidad (XSL) normalizado sigma_min
      qu10=0.0d0 !parametro calidad (XSL) aceptable 1.0 sigma
      qu05=0.0d0 !parametro calidad (XSL) aceptable 0.5 sigma
      qu15=0.0d0 !parametro calidad (XSL) aceptable 1.5 sigma
      starmin=3.0d0 !minimo aceptable estrellas: 3 (1 x subcaja)
      fja99i=0.0d0
cc U and missing U filter fluxes in MILES and XSL
cc	fluxUm=0.0d0 !Missing U MILES flux
cc	fluxUi=0.0d0 !Missing U XSL flux
cc	fluxU=0.0d0 !Total U flux
cc	fluUms=0.0d0 !Missing u sdss MILES flux
cc	fluUis=0.0d0 !Missing u sdss XSL flux
cc	fluUis004=0.0d0 !Missing u sdss XSL flux at z=0.04
...
cc       call fraU(tiss,giss,fUm,fUi)
cc       call fra_u_usdss(tiss,giss,fusdss)
cc       call frausdss(tiss,giss,fUms,fUis)
cc       call frausdss004(tiss,giss,fUis004)
       call sigmam_XSL_276A(tiss,giss,fiss,ssppi,ali) !XSL
       fvmil=0.0d0
       do l=1,npxi
        rfV=0.0d0
        if(l.eq.npxi)then
        ...
        if(ssppi(l,1).ge.V1.and.ssppi(l,1).le.V2)then
         ...
         if(rfV.gt.0.0d0) fvmil=fvmil+rfV*ssppi(l,2)*dl
        endif
       enddo
       IF(fvmil.gt.0.0d0)THEN
        DO l=1,npxi !XSL
         ssppi(l,2)=ssppi(l,2)/fvmil
         spfl=ssppi(l,2)*flujv
         if(m.eq.1)then
          if(imam(k).gt.0)then
           do mma=1,imam(k)
            wfluji(l,2)=wfluji(l,2)+R140(k,mma)*spfl
            wflujis(l,2)=wflujis(l,2)+R140(k,mma)*spfl*spfl
           enddo
          endif
          wfluji(l,2)=wfluji(l,2)+R(k,1,14)*spfl
          wflujis(l,2)=wflujis(l,2)+R(k,1,14)*spfl*spfl
         else
          wfluji(l,2)=wfluji(l,2)+R(k,m,14)*spfl
          wflujis(l,2)=wflujis(l,2)+R(k,m,14)*spfl*spfl
         endif
        ENDDO
       ENDIF
       if(m.eq.1)then
       ...
       IF(fvmil.gt.0.0d0)THEN
        ...
        ENDDO
c       Missing MILES and XSL U fluxes:
cc        IF(m.eq.1)THEN
