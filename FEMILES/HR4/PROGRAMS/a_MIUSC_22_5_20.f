...
c MIUSC
      PARAMETER (npxi=6672) !MIUSC #pixels in the spectra
      PARAMETER (nsti=1100) !MIUSC #stars in the library
      DIMENSION wfi(npxi,2),wwffio(npxi,2),wwffiy(npxi,2) !MIUSC
      DIMENSION wfis(npxi,2),wwffiso(npxi,2),wwffisy(npxi,2) !MIUSC
      DIMENSION a9io(3),a9iy(3),a90i(3) !MIUSC
      DIMENSION cabi(150,4) !cabecera MIUSC
...
c
     ...
     &vkvega,zerovk
      COMMON/Uflujs/fxUtm,fxUti,fxUt,fxUtms,fxUtis,fxUts,fxUtis004 !a,e
      COMMON/ab/iaba
...
245   format(A6)
      OPEN(98,FILE='../D/INPUT/PARAM_MIUSC',STATUS='OLD') !MIUSC
      nstari=0
      do k=1,99999
...
ccccccccccccccccccccc
	open(99,file='../D/INPUT/GALAXY_CABECERAS_MIUSC',status='old') !MIUSC
	do k=1,61
...
ccccccccccccccccccccc
	DO NNN=1,nnIMF
	 ...
	 if(shape.eq.'b')then
	  ...
	  chatom='./OUT/I/Ibi'//chato !MIUSC
	 elseif(shape.eq.'u')then
	  write(chato(1:4),'(f4.2)')ZMU-1.0d0
	  chatom='./OUT/I/Iun'//chato !MIUSC
         elseif(shape.eq.'g')then
          ...
          chatom='./OUT/I/Ig'//chato(1:5)
	 elseif(shape.eq.'k')then
	  write(chato(1:4),'(f4.2)')ZMU-1.0d0
	  chatom='./OUT/I/Iku'//chato !MIUSC
	 elseif(shape.eq.'r')then
	  write(chato(1:4),'(f4.2)')ZMU-1.0d0
	  chatom='./OUT/I/Ikb'//chato !MIUSC
	 elseif(shape.eq.'c')then
	  write(chato(1:4),'(f4.2)')ZMU-1.0d0
	  chatom='./OUT/I/Ich'//chato !MIUSC
	 elseif(shape.eq.'f')then
	  write(chato(1:4),'(f4.2)')ZMU-1.0d0
	  chatom='./OUT/I/IFe'//chato
	 elseif(shape.eq.'x')then
	  write(chato(1:4),'(f4.2)')ZMU-1.0d0
	  chatom='./OUT/I/IFx'//chato
	 elseif(shape.eq.'l')then
	  ...
	  if(ZMU1.ge.0.0d0)then
	   if(int1.lt.10)then
	    ...
	    chatom='./OUT/I/Ip0'//chato
	   else
	    write(chato(1:2),'(I2)')int1
	    chatom='./OUT/I/Ip'//chato
	   endif
	  else
	   if(int1.lt.10)then
	    write(chato(1:1),'(I1)')int1
	    chatom='./OUT/I/Im0'//chato
	   else
	    write(chato(1:2),'(I2)')int1
	    chatom='./OUT/I/Im'//chato
	   endif
	  endif
...
! 	  write(iru_str,'(i0)')iru
	  write(0,*)sfr_str,fga_str
	  call system
     &('python3 ./galIMF-master/igimf_calculator.py '//sfr_str//' '//fga_str//' d d d d')
...
c     ==================== IGIMF ===========================
	   DO iao=1,ntold
	    ageo=ttold(iao)
          call STU_MIUSC(izold(izo),ageo,BETAo,realeo,ZMISOo,wwffio,
     &wwffiso,a9io,f9mo,qumo,qu10o,qu05o,qu15o,fxUmo,fxUio,fxUo,
...
c*******COMIENZO DEL BUCLE DE LA SSPy**********************************
     ...
     &abs(pizyng(izyng(izy)))
		...
		agey=ttyng(iay)
	      ...
	      IF(alpha.ne.1.0d0)THEN
	     call STU_MIUSC(izyng(izy),agey,BETAy,realey,ZMISOy,wwffiy,
     &wwffisy,a9iy,f9my,qumy,qu10y,qu05y,qu15y,fxUmy,fxUiy,fxUy,
...
c		 write(77,*)wfis(lup,1),wfis(lup,2)/wfi(lup,2)
		 write(77,*)wfis(lup,1),wfis(lup,2)
	      ...
	      call e_MIUSC(alpha,izold(izo),realeo,izyng(izy),realey)
2325	      continue
