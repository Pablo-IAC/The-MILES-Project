c*******SUBRUTINA DE Fe4531************************************************
c        SUBROUTINE fe4531(nra,i,W,stars)
        SUBROUTINE fe4531(i,W,stars)
...
c        feh(i)=fez(i)+dlog10(ra(nra,3)/ra(1,3))
	spend=(4531.-4400.)/(5500.-4400.)
	W=0.0
	stars=0.0
	if(jotai(i).eq.0)goto 50005
	DO j=1,jotai(i)
	...
	ZMAG=-11.560+4.9033e-3*at-8.3331e-8*at**2
			elseif((lc.ge.4).and.(R(i,j,1).le.7.0))then
	ZMAG=9.9836-1.3061*R(i,j,1)+0.02782*R(i,j,1)**2
			else
				goto 10005
			endif
		elseif((R(i,j,3).ge.3.5527).and.(R(i,j,3).le.3.7076))then
	ZMAG=3.0244+21.3021*t+0.6231*feh(i)+0.0506*feh(i)**3+
     &  0.0056*R(i,j,4)**3+0.1341*feh(i)*R(i,j,4)
		elseif((R(i,j,3).gt.3.7076).and.(R(i,j,3).le.4.1225))then
	t=10**t
	ZMAG=-2.9636+6.2107*t+0.9795*t*feh(i)
		else
			goto 10005
		endif
        if(j.eq.1)then
